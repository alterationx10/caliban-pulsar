...
object Caliban extends App {
  ...
  val client: ZLayer[Any, Throwable, Has[PulsarClient]] = ZLayer.succeed(PulsarClientConfig(serviceUrl = s"pulsar://$host:6650"))  >>> PulsarClientProvider.layer
  ...
  val subservice: ZLayer[Any, Throwable, Has[SubscriptionService]] = consumer >>> SubscriptionService.live

  val deps: ZLayer[Any, Throwable, Has[UserEventRepository] with Has[Producer[String]] with Has[Consumer[String]] with Has[SubscriptionService]]
