...
object PulsarConsumerProvider {
  ...
  val managed: ZManaged[Has[PulsarClient] with Has[PulsarConsumerConfig], Nothing, Consumer[String]] =
    ZManaged.make(acquire)(_.release)
    
  val layer: ZLayer[Has[PulsarClient] with Has[PulsarConsumerConfig], Nothing, Has[Consumer[String]]] = ZLayer.fromManaged(managed)
}